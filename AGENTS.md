...
- `卡片素材/高解析/` 保存原始 PNG，供放大、編輯與印刷；不得直接作為網頁執行時圖片。
- `卡片素材/低解析/` 保存 480×670 WebP，供 HTML 與 GitHub Pages 載入。
- 網頁卡牌圖片路徑一律使用 `卡片素材/低解析/<分類>/`；不可再引用根目錄的 `星座卡/`、`功能卡/`、`特殊卡/`。
- 新增卡片時先保留高解析原圖，再產生低解析 WebP，並同步更新引用它的 HTML。
- 未來若加入卡片上傳功能，任何高解析上傳檔都必須先縮放／轉檔成低解析 WebP；目前 teach 僅發布既有素材與網頁，不含上傳介面。
- 路徑異動後以 `rg` 掃描所有 HTML 與 Markdown，並以 `git diff --check` 驗證。
